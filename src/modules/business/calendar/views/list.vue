<template>
  <b-container fluid>
    <main-modal id="calendarAddEditModal" size="lg">
      ...
      <template v-slot:actions v-if="typeOfModal == 'edit' || detailsStatus == true">
        <div class="d-flex">
              ...
              <div
                ...
                <div class="custom-switch-inner">
                  <input type="checkbox" class="custom-control-input bg-info" :id="'status'"
                         @change="changeStatus(calendarDetails.id, calendarDetails.status_traker)"
                         v-model="calendarDetails.status_traker">
                    ...
                    <ul class="my-ul">
                      <li class="d-flex justify-content-between gap-1">
                        ...
                        <b-dropdown class="actions-dropdown">
                          ...
                          <b-dropdown-item-button @click="() => { calendarDetailsFront = slot; deleteSlot()}">
                            <!-- <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  methods: {
    ...
    deleteSlot () {
      EventBus.$emit('openDeleteModal', {
        actionHeader: 'Clear',
        titleHeader: 'Slot',
        textContent: `${this.calendarDetailsFront.service.name} in ${this.calendarDetailsFront.day} `,
        question: 'Are you sure you want to delete this slot from the calendar?',
    ...
    loadSchedule () {
      this.requestLoading = true
      const payload = { from: this.getDays[0], to: this.getDays[6] }
      calendarServices.loadSchedule(payload).then(res => {
        core.showSnackbar('success', res.data.message)
        this.getCalendar()
      }).finally(() => {
        this.requestLoading = false
    ...
    changeStatus (id, status) {
      ...
      mainService.changeStatus(obj).then(res => {
        this.calendarDetails.status = obj.status
        this.getCalendar()
        core.showSnackbar('success', res.data.message)
      }).catch(() => {
        this.calendarDetails.status = !status
      })
    },
    formatTime (time) {
