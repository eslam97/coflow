...
export default {
  ...
  methods: {
    ...
    getInvitationsMemberReport () {
      this.loadTable = true
      services.getAllInvitations({ pageNumber: this.currentPage, userName: this.filter.search, dateFrom: this.dateRange.startDate ? new Date(this.dateRange.startDate).toJSON().slice(0, 10) : '', dateTo: this.dateRange.endDate ? new Date(this.dateRange.endDate).toJSON().slice(0, 10) : '', memberId: '' }).then(res => {
