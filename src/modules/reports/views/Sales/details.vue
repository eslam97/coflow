      ...
      <template v-slot:body>
        ...
        <b-table v-else striped responsive   :fields="columns" :items="salesReport" class="mb-0 table-borderless" select-mode="single" ref="selectableTable" sort-icon-left primary-key="name"  id="table-transition-example" :tbody-transition-props="transProps">
          ...
          <template v-slot:cell(call_amount)="data">
            <router-link class="linkedReport" :to="{name: 'salesReportHistories', query:  {
                                       user : data.item.id,
                                       project: filter.project,
                                      startDate: dateRange.startDate ?new Date(dateRange.startDate).toJSON().slice(0,10) : '',
                                       endDate: dateRange.endDate ? new Date(dateRange.endDate).toJSON().slice(0, 10) : ''
                                     }}"><b-badge variant="info">{{ data.item.call_amount}}</b-badge></router-link>
          </template>
          <template v-slot:cell(contract_amount)="data">
            <router-link class="linkedReport" :to="{name: 'contract_amount', query:  {
                                       user : data.item.id,
                                       project: filter.project,
                                      startDate: dateRange.startDate ?new Date(dateRange.startDate).toJSON().slice(0,10) : '',
                                       endDate: dateRange.endDate ? new Date(dateRange.endDate).toJSON().slice(0, 10) : ''
          ...
          <template v-slot:cell(achieved_amount)="data">
            <router-link class="linkedReport" :to="{name: 'salesReportAchieved', query:  {
                                       ...
                                       startDate: dateRange.startDate ?new Date(dateRange.startDate).toJSON().slice(0,10) : '',
                                       endDate: dateRange.endDate ? new Date(dateRange.endDate).toJSON().slice(0, 10) : ''
                                     }}"><b-badge variant="danger">{{ data.item.achieved_amount}}</b-badge></router-link>
          </template>
        </b-table>
