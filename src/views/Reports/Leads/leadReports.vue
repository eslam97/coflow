...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { label: 'Code', key: 'id', class: 'text-left' },
  ...
  methods: {
    ...
    getLeadReport () {
      this.loadTable = true
      this.$store.dispatch('getLeadReport', { pageNumber: this.currentPage, search: this.filter.search, project: this.filter.project, userId: this.filter.userId, callStatusId: this.filter.callStatusId, inquiry: this.filter.inquiry, source: this.filter.source, from: this.filter.from, dateFrom: this.dateRange.startDate ? new Date(this.dateRange.startDate).toJSON().slice(0, 10) : '', dateTo: this.dateRange.endDate ? new Date(this.dateRange.endDate).toJSON().slice(0, 10) : '', ifNew: this.filter.noHistory ? 1 : 0, category: this.filter.category, noHistory: this.filter.noHistory ? 1 : 0, userid: this.filter.user_id, takenId: this.filter.taken_id }).then(res => {
