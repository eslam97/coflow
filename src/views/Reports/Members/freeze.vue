    ...
    <div class="iq-card p-1">
      <div class="row">
        <div class="col-md-12 my-1">
          <input type="text" v-model="filter.search" placeholder="search by name or phone" class="form-control" @input="getFreezeMemberReport">
        </div>
        <div class="col-md-12 my-1">
          <date-range-picker
              class="w-100"
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { label: 'End', key: 'end' },
        { label: 'Freezing Weeks', key: 'member_package.freezing_weeks' },
  ...
  methods: {
    ...
    getFreezeMemberReport () {
      this.loadTable = true
      this.$store.dispatch('getAllFreeze', { pageNumber: this.currentPage, userName: this.filter.search, dateFrom: this.dateRange.startDate ? new Date(this.dateRange.startDate).toJSON().slice(0, 10) : '', dateTo: this.dateRange.endDate ? new Date(this.dateRange.endDate).toJSON().slice(0, 10) : '' }).then(res => {
