    ...
    <div class="iq-card p-1">
      <div class="row">
        <div class="col-md-12 my-1">
          <input type="text" v-model="filter.search" placeholder="search by name or phone" class="form-control" @input="getVisitsMemberReport">
        </div>
        <div class="col-md-12 my-1">
          <date-range-picker
              class="w-100"
...
<!--            <img v-if="data1.item.image" :src="data1.item.profile" class="avatar-50 rounded-circle"/>-->
          ...
          <template v-slot:cell(services)="data">
            <ul class="m-0 p-0">
              <li v-for="(i, key) in data.item.activity" :key="key">
                <span v-for="(service, key) in i.services" :key="key"> {{service.service_title}}
                </span>
              </li>
            </ul>
          </template>
          <template v-slot:cell(check_in)="data">
            {{data.item.check_in?data.item.check_in.substring(0,10): ''}} <span class="text-success">{{tConvert(data.item.check_in?data.item.check_in.substring(11): '')}}</span>
          </template>
          <template v-slot:cell(check_out)="data">
            {{data.item.check_out?data.item.check_out.substring(0,10): ''}} <span class="text-danger">{{tConvert(data.item.check_out?data.item.check_out.substring(11): '')}}</span>
          </template>
        </b-table>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { label: 'Profile', key: 'member.profile' },
        { label: 'Name', key: 'member.name' },
        { label: 'ID', key: 'member.membership_id' },
        { label: 'Date', key: 'date' },
  ...
  methods: {
    ...
    getVisitsMemberReport () {
      this.loadTable = true
      this.$store.dispatch('getAllVisits', { pageNumber: this.currentPage, userName: this.filter.search, dateFrom: this.dateRange.startDate ? new Date(this.dateRange.startDate).toJSON().slice(0, 10) : '', dateTo: this.dateRange.endDate ? new Date(this.dateRange.endDate).toJSON().slice(0, 10) : '' }).then(res => {
