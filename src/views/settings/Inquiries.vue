<template>
  <b-row>
    <b-modal id="modal-5" size="md" title="Remove inquiries" hide-footer no-close-on-backdrop>
      <p class="font-size-18 text-black">Are you sure you want to delete this inquiries? </p>
      <div class="d-flex justify-content-end">
        <b-button variant="danger" @click="deleteinquiries">Delete inquiries</b-button>
      </div>
    </b-modal>
      ...
      <div class="iq-card p-1">
        <div class="row">
          <div class="col-md-3 my-1">
            <input type="text" placeholder="search by Inquiries name" class="form-control" v-model="filter.title" @input="filterTable" >
          </div>
          <div class="col-md-3 my-1">
    ...
    <b-col lg="8">
      <iq-card>
        <template v-slot:headerTitle>
          <h4 class="card-title">{{ $t('dashboard2.listinquiries')}}</h4>
        </template>
        <template v-slot:headerAction>
          ...
          <b-button v-else v-b-modal.modal-inquiries variant="primary" @click="loadData">Add inquiries</b-button>
          <b-modal id="modal-inquiries" size="lg"  :title="modelTitle" hide-footer no-close-on-backdrop>
            <addinquiriesModal :edit="editFlag"/>
          </b-modal>
        </template>
        <template v-slot:body>
          ...
          <b-table v-else striped responsive :fields="columns" :items="getAllinquiries" class="mb-0 table-borderless">
...
const addinquiriesModal = () => import('../../components/inquiries/inquiriesModal')
export default {
  ...
  methods: {
    ...
    filterTable () {
      this.loadTable = true
      this.$store.dispatch('getAllinquiries', { filterTitle: this.filter.title, filterStatus: this.filter.status }).then(data => {
    ...
    changeContent () {
      this.loadTable = true
      this.$store.dispatch('getAllinquiries', { pageNumber: this.currentPage }).then(() => {
        this.loadTable = false
      })
    },
    switchStatus (id) {
      this.$store.dispatch('changeStatusinquiries', id).then(() => {
        core.showSnackbar('success', 'status inquiries has been Edit successfully.')
      })
    },
    loadData () {
      this.loadingOpenAdd = true
      this.editFlag = false
      this.modelTitle = 'Add inquiries'
    ...
    gitDataToEdit (id) {
      ...
      this.modelTitle = 'Edit inquiries'
      this.$store.dispatch('GetSpacificinquiries', id).then(() => {
        this.$bvModal.show('modal-inquiries')
    ...
    deleteinquiries () {
      this.$store.dispatch('deleteinquiries', this.deleteId).then(() => {
        this.$bvModal.hide('modal-5')
        core.showSnackbar('success', 'inquiries has been deleted successfully.')
      })
    },
    print (id) {
      // console.log(id)
  ...
  created () {
    this.loadTable = true
    this.$store.dispatch('getAllinquiries', { pageNumber: this.currentPage }).then(data => {
      this.currentPage = data.data.meta.currentPage
      this.per_page = data.data.meta.per_page
      this.total = data.data.meta.total
      this.loadTable = false
