<template>
  <b-row>
    <b-modal id="modal-5" size="md" title="Remove callStatus" hide-footer no-close-on-backdrop>
      <p class="font-size-18 text-black">Are you sure you want to delete this callStatus? </p>
      <div class="d-flex justify-content-end">
        <b-button variant="danger" @click="deletecallStatus">Delete callStatus</b-button>
      </div>
    </b-modal>
      ...
      <div class="iq-card p-1">
        <div class="row">
          <div class="col-md-3 my-1">
            <input type="text" placeholder="search by call status" class="form-control" v-model="filter.title" @input="filterTable" >
          </div>
          <div class="col-md-3 my-1">
    ...
    <b-col lg="8">
      <iq-card>
        <template v-slot:headerTitle>
          <h4 class="card-title">{{ $t('dashboard2.listcallStatus')}}</h4>
        </template>
        <template v-slot:headerAction>
          ...
          <b-button v-else v-b-modal.modal-callStatus variant="primary" @click="loadData">Add callStatus</b-button>
          <b-modal id="modal-callStatus" size="lg"  :title="modelTitle" hide-footer no-close-on-backdrop>
            <addcallStatusModal :edit="editFlag"/>
          </b-modal>
        </template>
        <template v-slot:body>
          ...
          <b-table striped v-else responsive :fields="columns" :items="getAllcallStatus" class="mb-0 table-borderless">
...
const addcallStatusModal = () => import('../../components/call-status/callstatusModal')
export default {
  name: 'settingscallStatus',
  data () {
    return {
      filter: {
        title: '',
      ...
      columns: [
        ...
        { label: 'Reserve', key: 'days', class: 'text-left' },
        { label: 'Hold', key: 'hold', class: 'text-left' },
        { label: 'Return', key: 'return', class: 'text-left' },
  ...
  methods: {
    ...
    filterTable () {
      this.loadTable = true
      this.$store.dispatch('getAllcallStatus', { filterTitle: this.filter.title, filterStatus: this.filter.status }).then(data => {
    ...
    changeContent () {
      this.loadTable = true
      this.$store.dispatch('getAllcallStatus', { pageNumber: this.currentPage }).then(() => {
        this.loadTable = false
      })
    },
    switchStatus (id) {
      this.$store.dispatch('changeStatuscallStatus', id).then(() => {
        core.showSnackbar('success', 'status callStatus has been Edit successfully.')
      })
    },
    loadData () {
      this.loadingOpenAdd = true
      this.editFlag = false
      this.modelTitle = 'Add callStatus'
    ...
    gitDataToEdit (id) {
      ...
      this.modelTitle = 'Edit callStatus'
      this.$store.dispatch('GetSpacificcallStatus', id).then(() => {
        this.$bvModal.show('modal-callStatus')
    ...
    deletecallStatus () {
      this.$store.dispatch('deletecallStatus', this.deleteId).then(() => {
        this.$bvModal.hide('modal-5')
        core.showSnackbar('success', 'callStatus has been deleted successfully.')
      })
    },
    print (id) {
      // console.log(id)
  ...
  created () {
    this.loadTable = true
    this.$store.dispatch('getAllcallStatus', { pageNumber: this.currentPage }).then(data => {
      this.currentPage = data.data.meta.currentPage
      this.per_page = data.data.meta.per_page
      this.total = data.data.meta.total
      this.loadTable = false
