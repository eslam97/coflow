<template>
  <b-row>
    <b-modal id="modal-5" size="md" title="Remove sources" hide-footer no-close-on-backdrop>
      <p class="font-size-18 text-black">Are you sure you want to delete this sources? </p>
      <div class="d-flex justify-content-end">
        <b-button variant="danger" @click="deletesources">Delete sources</b-button>
      </div>
    </b-modal>
      ...
      <div class="iq-card p-1">
        <div class="row">
          <div class="col-md-3 my-1">
            <input type="text" placeholder="search by Source name" class="form-control" v-model="filter.title" @input="filterTable" >
          </div>
          <div class="col-md-3 my-1">
    ...
    <b-col lg="8">
      <iq-card>
        <template v-slot:headerTitle>
          <h4 class="card-title">{{ $t('dashboard2.listsourcess')}}</h4>
        </template>
        <template v-slot:headerAction>
          <b-button v-b-modal.modal-source variant="primary" @click="loadData">Add sources</b-button>
          <b-modal id="modal-source" size="lg"  :title="modelTitle" hide-footer no-close-on-backdrop>
            <addsourcesModal :edit="editFlag"/>
          </b-modal>
        </template>
        <template v-slot:body>
          ...
          <b-table v-else striped responsive :fields="columns" :items="getAllsources" class="mb-0 table-borderless">
            ...
            <template v-slot:cell(icon)="data2">
              <span :class="data2.item.icon"></span>
            </template>
...
export default {
  ...
  data () {
    return {
      ...
      total: 6,
      loadTable: true,
      modelTitle: 'Add sources',
      editFlag: false,
      switchButton: [],
      deleteId: '',
      columns: [
        { label: 'Title', key: 'title', class: 'text-left' },
        { label: 'Icon', key: 'icon', class: 'text-left' },
  ...
  methods: {
    ...
    filterTable () {
      this.loadTable = true
      this.$store.dispatch('getAllsources', { filterTitle: this.filter.title, filterStatus: this.filter.status }).then(data => {
    ...
    changeContent () {
      this.loadTable = true
      this.$store.dispatch('getAllsources', { pageNumber: this.currentPage, filterTitle: this.filter.title, filterStatus: this.filter.status })
      this.loadTable = false
    },
    switchStatus (id) {
      this.$store.dispatch('changeStatussources', id).then(() => {
        core.showSnackbar('success', 'status sources has been Edit successfully.')
      })
    },
    loadData () {
      this.editFlag = false
      this.modelTitle = 'Add sources'
    },
    gitDataToEdit (id) {
      document.getElementsByClassName('input_' + id)[0].innerHTML = '<span class="spinner-grow spinner-grow-sm"></span>'
      this.editFlag = true
      // eslint-disable-next-line no-unused-expressions,no-sequences
      this.modelTitle = 'Edit sources'
      this.$store.dispatch('GetSpacificsources', id).then(() => {
        this.$bvModal.show('modal-source')
    ...
    deletesources () {
      this.$store.dispatch('deletesources', this.deleteId).then(() => {
  ...
  created () {
    this.loadTable = true
    this.$store.dispatch('getAllsources', { pageNumber: this.currentPage }).then(data => {
      this.currentPage = data.data.meta.currentPage
      this.per_page = data.data.meta.per_page
      this.total = data.data.meta.total
      this.loadTable = false
